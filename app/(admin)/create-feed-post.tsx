...
import type { PostType, FeedTier } from '@/types/feed.types';
...
export default function AdminCreateFeedPost() {
    const router = useRouter();
    const { user } = useAuth();
    const [content, setContent] = useState('');
    const [category, setCategory] = useState<PostType>('general');
    const [tier, setTier] = useState<FeedTier>('local');
    ...
    // Determine available tiers based on user role
    const isParishAdmin = user?.role === 'admin';
    const isMasterAdmin = user?.role === 'master_admin'; // Assuming this role exists

    const availableTiers: { label: string; value: FeedTier; description: string }[] = [
        { label: 'Local Parish', value: 'local', description: 'Visible to your parish only' },
        ...(isParishAdmin || isMasterAdmin
            ? [
                {
                    label: 'Archdiocese',
                    value: 'archdiocese' as FeedTier,
                    description: 'Visible to all parishes in your archdiocese',
                },
            ]
            : []),
        ...(isMasterAdmin
            ? [
                {
                    label: 'National',
                    value: 'national' as FeedTier,
                    description: 'Visible to all users nationwide',
                },
            ]
            : []),
    ];
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (media.length > 0) {
                ...
                if (photos.length > 0) {
                    const photoFiles = photos.map((p) => fetch(p.uri).then((r) => r.blob()));
    ...
    const getTierLabel = () => {
        const selected = availableTiers.find((t) => t.value === tier);
        return selected?.label || 'Select Tier';
    };
            ...
            <ScrollView style={styles.content} keyboardShouldPersistTaps="handled">
                ...
                {availableTiers.length > 1 && (
                    <View style={styles.section}>
                        <Text style={styles.sectionTitle}>Publishing Tier</Text>
                        <TouchableOpacity
                            style={styles.tierSelector}
                            onPress={() => setShowTierPicker(!showTierPicker)}
                        >
                            <View style={styles.tierSelectorContent}>
                                <Text style={styles.tierSelectorText}>{getTierLabel()}</Text>
                                <Text style={styles.tierSelectorDescription}>
                                    {availableTiers.find((t) => t.value === tier)?.description}
                                </Text>
                            </View>
                            ...
                            <View style={styles.tierOptions}>
                                {availableTiers.map((tierOption) => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            styles.tierOption,
                                            tier === tierOption.value && styles.tierOptionActive,
                                        ]}
                                        onPress={() => {
                                            setTier(tierOption.value);
                                    ...
                                    >
                                        <View>
                                            <Text
                                                style={[
                                                    ...
                                                    tier === tierOption.value && styles.tierOptionLabelActive,
                                                ]}
                                            >
                                                {tierOption.label}
                                            </Text>
                                        ...
                                        {tier === tierOption.value && (
                                            <View style={styles.tierOptionCheck}>
                                                <Text style={styles.tierOptionCheckText}>✓</Text>
                                            </View>
                                        )}
                                    </TouchableOpacity>
                                ))}
                ...
                <View style={styles.infoBox}>
                    <Text style={styles.infoText}>
                        {tier === 'local' && '📍 This post will be published to your Local Parish Feed.'}
                        {tier === 'archdiocese' &&
                            '🏛️ This post will be visible to all parishes in your archdiocese.'}
                        {tier === 'national' && '🇰🇪 This post will be visible to all users nationwide.'}
                    </Text>
                </View>
            </ScrollView>
        </SafeAreaView>
