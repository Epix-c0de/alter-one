...
export default function ManageLocalChurchesScreen() {
  ...
  const handleGenerateId = async () => {
    if (!name || !parishId) {
      Alert.alert('Error', 'Please enter a church name and parish ID.');
      return;
    }
    ...
    try {
      const { data, error } = await supabaseFunctions.functions.invoke('generate-id', {
        body: { type: 'local_church', name, parentId: parishId },
      });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Manage Local Churches</Text>
      </View>
      <View style={styles.form}>
        ...
        <TextInput
          style={styles.input}
          placeholder="Parish ID"
          value={parishId}
          onChangeText={setParishId}
          placeholderTextColor={Colors.light.textTertiary}
        />
